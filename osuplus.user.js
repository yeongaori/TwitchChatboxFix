...
(() => {
            ...
            $("<div id='osuplusModal' class='osuplus' style='display:none;'></div>").append(
                ...
                $("<div class='osuplusSettingsContent'>").append(
                    $("<div>").append(
                        ...
                        $("<table class='osuplusSettingsTable' width='100%'>").append(
                            ...
                            makeSettingRow("Click user profile picture to open image", null, makeCheckboxOption("clickPfp"))
                        )
                    ),
                    $("<div>").append(
                        "<h2>Beatmaps page</h2>",
                ...
                $("<button id='osuplusSettingsSaveBtn' style='background-color:rgba(0,0,0,0); border-color:rgba(0,0,0,0); color:#d9a6bd;'>Save</button>").click(function(){
                    ...
                    var properties = [
                        ...
                        "showDetailedHitCount", "showHitsPerPlay", "fetchUserpageMaxCombo", "fetchFirstsInfo", "rankingVisible", "forceShowDifficulties", "showSiteSwitcher", "showMpGrades", "showRecent", "clickPfp"
                    ];
    ...
    function osuplusNew(){
        ...
        function init(){
            ...
            if (settings.clickPfp) {
                const avatarElement = document.querySelector(".avatar.avatar--guest.avatar--full");
                if (avatarElement) {
                    avatarElement.addEventListener("click", openPfp);
                }
            }
            function openPfp() {
                var pfp = document.getElementsByClassName("avatar avatar--guest avatar--full")[0].getAttribute("style").split('url("')[1].split('"')[0];
                window.open(pfp, '_blank');
            }
            function reInit(){
        ...
        function addOsuPreview(){
            songInfoRef.parent().next().after(
                $("<div class='spoiler'></div>").html(
                    ...
                    $("#osupreview").html(
                        "If below doesn't work, <a href='http://osu.pages.dev/preview#" + mapID + "' target='_blank'>open in new tab</a><br>" +
                        "<iframe class='osupreview' src='https://osu.pages.dev/preview#" + mapID + "' allowfullscreen></iframe>"
                    );
